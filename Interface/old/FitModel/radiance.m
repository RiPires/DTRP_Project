...
%--------------------------------------------------------------------------
data = {};
TestData = {true 'Data1.m' 128 53.6 4.88 28;... %N,D,d,T
            true 'Data2.m' 35 61.5 1.5 42;...
            true 'Data3.m' 83 55 1.8 37;...
            true 'Data4.m' 51 45 1.8 37;...
            true 'Data5.m' 24 32.5 1.8 37};
...
% INTERFACE LOGO
%--------------------------------------------------------------------------                                                   ----------%
[tmp,~,alfa] = imread('logo2.png');

handles.axes_logo = axes('Parent',mainFig,...
                         'Units','Normalized', ...
                         'Position',[0.001 0.93 0.12 0.07],...
                         'Visible','off');
tmp=image(tmp);
set(tmp, 'AlphaData', alfa);
axis equal off
...
handles.uitable = uitable(handles.panel_DataDetails,...        
    ...
    'ColumnFormat',     {'logical' 'char' 'numeric' 'numeric' 'numeric' 'numeric' {'red' 'green' 'blue' 'cyan' ...
    'magenta' 'yellow' 'black' 'orange' 'purple' 'pink' 'gray' 'bordeaux' 'dark green' 'dark yellow'} ...
    {'circle' 'plus sign' 'asterik' 'point' 'cross' 'horizontal line' 'vertical line' 'square' 'diamond' ...
    'upward-pointing triangle' 'downward-pointing triangle' 'right-pointing triangle' 'left-pointing triangle' ...
    'pentagram' 'hexagram'} 'Button'},...                                     
...
% Cell array to store the buttons for each row
numRows = size(TestData, 1);  % Get the number of rows in the TestData
buttonColumn = cell(numRows, 1);

% Populate the button column with 'Delete' buttons
for row = 1:numRows
    buttonColumn{row} = uicontrol('Parent', handles.uitable, ...
        'Style', 'pushbutton', ...
        'String', 'Delete', ...
        'UserData', row, ...  % Store the row number in UserData to identify which row to delete
        'Callback', @deleteRowCallback ...
    );
end

% Set the 'Button' column data
set(handles.uitable, 'Data', [TestData, buttonColumn]);

% Add the callback function to handle the button click
function deleteRowCallback(hObject, ~)
    handles = guidata(hObject);  % Get the handles structure from the hObject
    tmp = get(handles.uitable, 'Data');  % Get the current data from the table
    row = get(hObject, 'UserData');  % Get the row number from the UserData of the button
    
    % Remove the selected row from the table
    tmp(row, :) = [];
    
    % Update the table data and re-populate the 'Button' column
    numUpdatedRows = size(tmp, 1);
    buttonColumn = cell(numUpdatedRows, 1);
    for updatedRow = 1:numUpdatedRows
        buttonColumn{updatedRow} = uicontrol('Parent', handles.uitable, ...
            'Style', 'pushbutton', ...
            'String', 'Delete', ...
            'UserData', updatedRow, ...
            'Callback', @deleteRowCallback ...
    ...
    set(handles.uitable, 'Data', tmp, 'ColumnFormat', ...
        {'logical', 'char', 'numeric', 'numeric', 'numeric', 'numeric', ...
        {'red', 'green', 'blue', 'cyan', 'magenta', 'yellow', 'black', 'orange', 'purple', 'pink', ...
        'gray', 'bordeaux', 'dark green', 'dark yellow'}, ...
        {'circle', 'plus sign', 'asterisk', 'point', 'cross', 'horizontal line', 'vertical line', 'square', ...
        'diamond', 'upward-pointing triangle', 'downward-pointing triangle', 'right-pointing triangle', ...
        'left-pointing triangle', 'pentagram', 'hexagram'}, 'Button'}, ...
        'ColumnName', {'Select', 'Author', 'Nr. Patients', 'D [Gy]', 'd [Gy/fx]', 'T [days]', 'Color', 'Marker', 'Delete'}, ...
        'CellEditCallback', @datauitable_Callback);
end
...
%---------------------------------------------------------------
% BUTTONS FOR THE PLOTS THE USER WANTS TO SEE (MAXIMUM OF 2)
%---------------------------------------------------------------
...
handles.pushbutton_Fit = uicontrol(panel_ModelOpt,...
    ...
    function pushbutton_Fit_Callback(~,~)
        ...
        if isempty(data)                                     % If the table is empty
            helpdlg('No data to perform fit(s)!','')            % nothing to display
...
%----------------------------
handles.uitable_ResultFit1 = uitable(handles.panel_ResultsFit1,...
...
%----------------------------
handles.uitable_ResultFit2 = uitable(handles.panel_ResultsFit2,...
