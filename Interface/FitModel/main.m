...
uipushtool(handles.toolbar, ...
    ...
    'Enable',           'on',...
    'ClickedCallback',  @pushbutton_Callback)
...
handles.pushbutton_add = uicontrol(panel_DataList,...
    ...
    'Tooltipstring',    'Add a new data set',...
    ...
    function pushbutton_add_Callback(~,~)
        ...
        DataSetsList = get(handles.listbox_DataSets,'String');  % Get entries in listbox_DataSets
        n=numel(DataSetsList); 
        
        if isempty(DataSetsList)                                % if there is no data yet in the list or all was deleted
            file = uigetfile;                                   % Selects file
            FileName = strsplit(file, '.');
            FileName = string(FileName{1});                     % Gets file name
            set(handles.listbox_DataSets,...
                'String',   FileName,...
            ...
            file = uigetfile;                                    % User selects file
            FileName = strsplit(file, '.');
            FileName = string(FileName{1});                      % Gets file name
...
%-----------------------------
handles.uitable = olduitable(handles.panel_DataDetails,...          % the olduitable allows to ToolTip the header
    'Tag',              'uitable',...
    ...
    'ColumnToolTip',    {'Select data to perform the fit'...        % Tips when mouse hover
                         ...
                         'Dose per fraction'...
                         'Treatment time'},...
    'CellEditCallback',  @listbox_DataSets_Callback);
%----------                                                     ----------%
...
handles.pushbutton_fit = uicontrol(panel_Fit,...
    ...
    function pushbutton_Fit_Callback(~,~)
        ...
        if strcmp(state1, 'on')
            ...
            FitData = {round(str2double(K),5) round(str2double(alpha),5) round(str2double(beta),5)...
                       round(str2double(alpha)/str2double(beta),3) round(str2double(gamma),5)...
                       round(str2double(a),5) round(str2double(Td),0) round(str2double(delta),5)};
            set(handles.fiteduitable1,...
                'Data', FitData,...
                'Userdata', FitData)
            %msgbox('Complete!')
        elseif strcmp(state2, 'on')
            [K50_K0, alpha, beta, gamma, Td, sigmak_k0, delta] = fit2(file_names, Labels);
            FitData = {RoundStr2Double(K50_K0,5)...
                       RoundStr2Double(alpha,5)...
                       RoundStr2Double(beta,5)...
