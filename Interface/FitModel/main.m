...
handles.uimenu_file = uimenu(mainFig, ...   % Menu "File"
    ...
    handles.uimenu_saveFile = uimenu(handles.uimenu_file,...
        ...
        function uimenu_save_callback(~,~)        
            ...
            uisave(get(handles.uitable,'Data'),'ListStudents.mat') %Suggested filename
...
handles.uitable = uitable(handles.panel_DataDetails,...        
    ...
    function datauitable_Callback(hObject, event)
            ...
            if ~isnan(input)                            % check if it's not NaN 
                ...
                if (indices(2) == 4 || indices(2) == 7)... % If a cell in columns 4 or 7 changes
                    ...
                    warndlg(['Are you sure this is the correct input? ' ...
                             'Tipicaly the number of patients in the study and the total treament time are positive, ' ...
                ...
                elseif (indices(2) == 4 || indices(2) == 5 ||... % If a cell in the columns 4, 5,
                    ...
                    errordlg(['Number of patients in the study, total prescription dose, ' ...
        ...
        elseif indices(2) == 1                          % If a cell in column 1 changes (Selection state)
...
handles.fit1optionsuitable = uitable(panel_Fit,...
    ...
    'ColumnName',       {'K' '<html><center />&alpha<br /> [Gy<sup>-1</sup>]</html>' ...
                         '<html><center />&beta<br />[Gy<sup>-2</sup>]</html>' '<html><center />&gamma<br />[days<sup>-1</sup>]</html>' ...
...
handles.fit2optionsuitable = uitable(panel_Fit,...
    ...
    function fitoptionsuitable_Callback(hObject,event)        
        ...
        if ~isnan(input)                                 % In the case it's a number                               
            ...
            if input <0                                  % Check if it's not negative
...
handles.pushbutton_fit = uicontrol(panel_Fit,...
    ...
    function pushbutton_Fit_Callback(~,~)
        ...
        if isempty(data)                                     % If the table is empty
            helpdlg('No data to show!','')                   % nothing to display
        ...
        if strcmp(state1, 'on')                                                     % If Fit1 model it's selected
            ...
            [p,up] = fit1(file_names, data, Bounds);                                % Performs Fit1 (p->vector with parameters; up->vector with uncertainties)
        ...
        elseif strcmp(state2, 'on')                                     % If Fit2 model is selected
            ...
            [p,up] = fit2(file_names, data, Bounds);                    % Performs Fit2
...
handles.fiteduitable1 = uitable(handles.panel_Fited,...
    ...
    'ColumnName',       {'K' ...
                         ...
                         '<html><center />&gamma <br /> [days<sup>-1</sup>]</html>' ...
                         '<html><center>T<sub>d</sub><br />[days]</html>' ...
...
handles.fiteduitable2 = uitable(handles.panel_Fited,...
    ...
    function pushbutton_SavePlot_Callback (~,~)
        ...
        newAxesBED = copyobj(handles.axes_BED,fignew);                      % Copy the appropriate axes
        set(newAxesBED,'Position', [.25 .0 .5 .5], 'Units', 'normalized');  % The original position is copied too, so adjust it.
        
        newAxesTime = copyobj(handles.axes_Time, fignew);
        set(newAxesTime, 'Position', [.25 .7 .5 .5], 'Units', 'normalized');
