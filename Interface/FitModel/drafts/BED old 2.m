...
% -------------------------------------------------------------------------
    ...
    % we store these points for each year selected by the user accompanied by their
    % error bars
    points1y = {};
    points2y = {};
    points3y = {};
    ...
    % array with the months values in crescent order
    months_values = taus;
    ...
    for m = 1:length(months_values)
        ...
        for s = 1:number_studies 
            ...
            for t = 1:size(tau)
                if abs(month-tau(t)) < 2 % if there is a tau that corresponds to the year in question
    ...
    d = dref; %dose per fraction in Gy/fx
