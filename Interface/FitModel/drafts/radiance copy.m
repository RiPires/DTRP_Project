...
%--------------------------------------------------------------------------
data = {};
TestData = {false 'Data1.m' 128 53.6 4.88 28 'magenta' 'v' 'X';... %N,D,d,T
            false 'Data2.m' 35 61.5 1.5 42 'blue' 'o' 'X';...
            false 'Data3.m' 83 55 1.8 37 'black' '^' 'X';...
            false 'Data4.m' 51 45 1.8 37 'red' 'square' 'X';...
            false 'Data5.m' 24 32.5 1.8 37 'green' 'o' 'X'};
...
% CIENCIAS LOGO
%--------------------------------------------------------------------------
[tmp,~,alfa] = imread('./Icons/cienciaslogo.png');
...
% INTERFACE LOGO
%--------------------------------------------------------------------------                                                   ----------%
[tmp,~,alfa] = imread('./Icons/logo.png');
...
% "Add File" Button
%-------------------
myimage = imread('./Icons/AddFileIcon.png');
...
%------------------------
myimage = imread('./Icons/DeleteFilesIcon.png');
myIcon = imresize(myimage, [20, 20]);
uipushtool(handles.toolbar, ...
...
% "Save Plot" Button
%---------------------
myimage = imread('./Icons/SavePlotIcon.png');
myIcon = imresize(myimage, [20, 20]);
uipushtool(handles.toolbar, ...
...
% "Clear Axes" Button
%---------------------
myimage = imread('./Icons/ClearPlotIcon.png');
myIcon = imresize(myimage, [20, 20]);
uipushtool(handles.toolbar, ...
...
handles.uitable = uitable(handles.panel_DataDetails,...        
    ...
    % Select All Button
    btnSelectUnAll = uicontrol(mainFig, 'Style', 'pushbutton', 'String', 'Select All', ...
...
handles.pushbutton_Fit = uicontrol(panel_ModelOpt,...
    ...
    function pushbutton_Fit_Callback(~,~)
        ...
        elseif any(cellstate(:,4)) || any(cellstate(:,5)) || ... % Check if there are any empty cells
            ...
            % CHANGE FOR A UNIQUE FILE
            noData = 'true';                                                          % Variable to make sure there is data to perform the fits
            for i = 1:numel(Selected)                       
                if isequal(Selected(i),{1})                                                 % If it's selected
                    ...
                    noData = 'false';                                                 % There is data to perform the fits
                end
            end
        end
       
        if strcmp(state1, 'on') && strcmp(noData, 'false')                           % If Fit1 model it's selected
        ...
        elseif strcmp(state2, 'on') && strcmp(noData, 'false')                      % If Fit2 model is selected
...
% when the button is clicked it opens a new window

plots = false; % variable to store if the user selected the plots or not

function pushbutton_Plots_Callback(~,plots)
    % Create a new UI figure for the selection window
    ...
    % Fit2
    handles.PrimarySelect_Fit2 = uicontrol('Parent', secondaryPanelFit2, ...
        'Tag', 'PrimarySelect_Fit2', ...
        'Style', 'checkbox', ...
        'String', 'Fit2', ...
        'Units', 'Normalized', ...
        'Position', [.05 .6 .9 .3], ...
    ...
    % BED
    handles.PrimarySelect_BED = uicontrol('Parent', secondaryPanelFit2, ...
        'Tag', 'PrimarySelect_BED', ...
        'Style', 'checkbox', ...
        'String', 'BED', ...
        'Units', 'Normalized', ...
        'Position', [.05 .3 .9 .3], ...
    ...
    % EQD2
    handles.PrimarySelect_EQD2 = uicontrol('Parent', secondaryPanelFit2, ...
        'Tag', 'PrimarySelect_EQD2', ...
        'Style', 'checkbox', ...
        'String', 'EQD2', ...
        'Units', 'Normalized', ...
        'Position', [.05 0 .9 .3], ...
    ...
    selectPlotsButton = uicontrol('Parent', selectionFig, ...
        ...
        'ForegroundColor',  "k",...
        'BackgroundColor',  '#D95319',...
        'Callback', @(src, event) SelectPlots_Callback(src, handles));

    % Callback for "Select Plot(s)" button
    function SelectPlots_Callback(~, handles)
        ...
        numSelectedFit2 = sum([
            handles.PrimarySelect_Fit2.Value
            handles.PrimarySelect_BED.Value
            handles.PrimarySelect_EQD2.Value
        ]);
        ...
        elseif numSelectedFit1 + numSelectedFit2 > 2
            ...
            msg = 'Your selection is valid.';
            plots = true; % variable to store if the user selected the plots or not
        end
    
        % Display the message
        msgbox(msg, 'Plot Selection');

        % List with the fits the user wants to perform
    
        if numSelectedFit1 == 1 && numSelectedFit2 == 1
            ListFits = [1,2];
        elseif numSelectedFit1 == 2
            ListFits = [1];
        elseif numSelectedFit2 == 2
            ListFits = [2];
        end
    end
    ...
    if numel(tmp) == 0                      % If there are no data
        ...
        for k = 1:numel(tmp)
            if cell2mat(select(k)) == 1 
                ...
                colorIndex = find(strcmp(colorMap(1, :), selectedColor)); % Find the index of the selected color in the list
